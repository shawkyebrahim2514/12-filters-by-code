...
//-------------------------------------------------
void loadImage (string paragraph, unsigned char img_arr[SIZE][SIZE][RGB]) {
    ...
    if(readRGBBMP(imageFileName, img_arr)){
...
void merge_img() {
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            for (int k = 0; k < RGB; ++k) {
                image2[i][j][k] = (image[i][j][k] + image2[i][j][k])/2;
            }
        }
    }
}
...
void darken_lighten() {
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j< SIZE; j++) {
            for (int k = 0; k < RGB; ++k) {
                if(lighten) {
                    image2[i][j][k] = min(255, image[i][j][k] + amount);
                }else{
                    image2[i][j][k] = max(0, image[i][j][k] - amount);
                }
            }
        }
    }
